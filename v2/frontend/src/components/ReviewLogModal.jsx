...
import './EvalLogModal.css'  // reuse same styles

export default function ReviewLogModal({ username, onClose, onStatusUpdate }) {
  ...
  // SSE log stream
  useEffect(() => {
    const es = new EventSource(`/api/reviews/stream/${username}`)
    esRef.current = es
    es.onmessage = (e) => {
      try {
  ...
  const handleStop = async () => {
    setStopping(true)
    try {
      await fetch('/api/reviews/stop', { method: 'POST' })
    } catch {}
  }

  const progress = status && status.total_queued > 0
    ? Math.round(((status.processed + status.failed) / status.total_queued) * 100)
    : null
      ...
      <div className="eval-log-modal" onClick={e => e.stopPropagation()}>
        <div className="eval-log-header">
          <div className="eval-log-title">
            {status?.running
              ? <span className="eval-status-running">⟳ GENERATING REVIEWS</span>
              : <span className="eval-status-done">✓ REVIEWS COMPLETE</span>
            }
          </div>
        ...
        {status && (
          <div className="eval-stats">
            <div className="eval-stat">
              <span className="eval-stat-label">Reviewed</span>
              <span className="eval-stat-value green">{status.processed}</span>
            </div>
            <div className="eval-stat">
            ...
            <div className="eval-stat">
              ...
              <span className="eval-stat-value">{status.unreviewed ?? '—'}</span>
            </div>
        ...
        >
          {logs.length === 0 ? (
            ...
            logs.map((line, i) => (
              <div
                key={i}
                className={`eval-log-line ${line.includes('✗') ? 'error' : line.includes('✓') ? 'success' : ''}`}
              >
                {line}
              </div>
            ))
